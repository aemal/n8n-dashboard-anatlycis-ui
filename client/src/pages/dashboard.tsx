...
export default function Dashboard() {
  ...
  const {
    ...
    queryKey: ["/api/executions/daily"],
  });

  const handleRefresh = () => {
    queryClient.invalidateQueries({ queryKey: ["/api/executions"] });
    queryClient.invalidateQueries({ queryKey: ["/api/executions/stats"] });
    queryClient.invalidateQueries({ queryKey: ["/api/executions/daily"] });
    refetchExecutions();
  };
          ...
          {hasError && (
            <div className="rounded-lg border border-amber-200 bg-amber-50 dark:border-amber-900 dark:bg-amber-950/30 p-4">
              ...
              <p className="mt-1 text-sm text-amber-700 dark:text-amber-300">
                Unable to connect to the data source. Please ensure Supabase credentials are configured.
              </p>
            </div>
          )}
